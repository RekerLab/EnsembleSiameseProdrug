...
class SiameseNN(nn.Module):
    def __init__(self, input_dim, hidden_size, output_size, attention_size, dropout_rate):
        ...
        self.internal_process_1 = InternalProcessing(hidden_size, hidden_size, dropout_rate)
        self.internal_process_2 = InternalProcessing(hidden_size, hidden_size, dropout_rate)
        self.fc2 = nn.Linear(hidden_size * 2, 1)
        self.output = nn.Sigmoid()

    def forward_one_1(self, x):
        x = self.fc1_1(x)
        x = self.internal_process_1(x)
        return x

    def forward_one_2(self, x):
        x = self.fc1_2(x)
        x = self.internal_process_2(x)
        return x

    def forward(self, reactant, product):
        reactant_out = self.forward_one_1(reactant)
        product_out = self.forward_one_2(product)
...
train_dataset_bt = pd.read_csv(base_path + '/reps/random_both_dataset_under{}_rep1.csv'.format(split))
...
pd.DataFrame(predictions).to_csv(base_path + '/results/ablation/siamese_{}_preds_{}_unpaired.csv'.format(test_name.split('_')[0],split))