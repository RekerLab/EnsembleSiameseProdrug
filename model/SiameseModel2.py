...
class SiameseDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        x1, x2 = self.X[index]
        y = self.y[index]
        # convert numpy arrays to PyTorch tensors; ensure same data type
...
pd.DataFrame(predictions).to_csv(base_path + '/results/siamese_{}_preds_{}.csv'.format(test_name.split('_')[0],split))
