...
def model_builder(x_dat, ext, modelType):
  ...
  if modelType == 'DT':
    model = DecisionTreeClassifier(max_depth = None, max_features = 'sqrt', criterion = 'gini', random_state=42)
  if modelType == 'GB':
    model = GradientBoostingClassifier(learning_rate = 0.5, max_depth = 5, n_estimators = 200, random_state=42)
  if modelType == 'XGB':
    model = xgb.XGBClassifier(booster = 'gbtree',eta = 1, max_depth = 6, n_estimators = 500, random_state=42)
