...
def model_builder(x_dat, ext, modelType):
  ...
  '''
  if modelType == 'RF':
    model = RandomForestClassifier(max_depth = None, max_features = 'sqrt', n_estimators = 500, random_state=42)
  if modelType == 'DT':
    model = DecisionTreeClassifier(max_depth = 50, max_features = 'sqrt', criterion = 'entropy', random_state=42)
  if modelType == 'GB':
    model = GradientBoostingClassifier(learning_rate = 0.5, max_depth = 4, n_estimators = 500, random_state=42)
  if modelType == 'XGB':
    model = xgb.XGBClassifier(booster = 'gbtree',eta = 0.5, max_depth = 6, n_estimators = 500, random_state=42)
  if modelType == 'SVM_Lin':
    model = svm.SVC(kernel = 'linear', probability = True, C = 0.1, gamma = 'scale', random_state=42)
  if modelType == 'SVM_RBF':
    model = svm.SVC(kernel = 'rbf', probability = True, C = 10, gamma = 0.01, random_state=42)
  if modelType == 'Log':
